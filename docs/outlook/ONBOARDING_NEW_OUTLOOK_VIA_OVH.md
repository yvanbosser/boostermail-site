...
> **Dernière mise à jour** : 05/05/2026 — session « Échéances V2 SaaS » : audit complet feature, spec consolidée [`SPEC_ECHEANCES_BOOSTERMAIL.md`](../specs_proto/SPEC_ECHEANCES_BOOSTERMAIL.md) (13 sections, source de vérité unique), gaps §10.1+§10.2 close (relance via mailto: + aperçu mail original via modale locale). Garde-fou nouveau **I-SESS-05** dans `audit/INVARIANTS.md` + `cloture_check.sh` pour empêcher les sessions futures de tomber dans le piège `OneDrive\Desktop\EasyMail\` (vestige obsolète depuis 12/04). Voir bilan [`OUTLOOK_BILAN_SESSION_20260505_echeances.md`](../sessions/OUTLOOK_BILAN_SESSION_20260505_echeances.md). **Reste à faire prochaine session** : §10.3 scheduler J-1 ouvré (vrai chantier, ~1 jour, reco endpoint cron). 03/05 antérieur : bilan factures Anthropic ([`OUTLOOK_BILAN_SESSION_20260503.md`](../sessions/OUTLOOK_BILAN_SESSION_20260503.md)).
