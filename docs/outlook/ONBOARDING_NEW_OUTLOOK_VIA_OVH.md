...
> **Dernière mise à jour** : 23/05/2026 — session **double cadrage** (réponse depuis sous-dossier + drag and drop PJ). 2 nouveaux docs V12 + 15 invariants ajoutés (`I-CLASSED-01` à `06` + `I-DRAGDROP-01` à `09`). Voir bilan [`OUTLOOK_BILAN_SESSION_20260523_cadrages_drag_drop_et_sous_dossier.md`](../sessions/OUTLOOK_BILAN_SESSION_20260523_cadrages_drag_drop_et_sous_dossier.md).
>
> **Dernière mise à jour précédente** : 08/05/2026 PM tardif — session **« Audit remediation — exécution du plan en 7 phases + 3 audits sous angles différents + 16 corrections »** (12 commits sur sub-branche `feat/yvan/audit-remediation-08-05` mergée `--no-ff` sur `feat/yvan/frontend`, top `55f3fc5`). Phase 1 bloquants SaaS livrée (PII redaction Blocs A/B/C + brief sanitization + cascade Haiku→Sonnet + SECURITY_GUARD étendu). Phase 2-4 quality + token optim (~−200 à −500 tokens/draft). Phase 5 test runner permanent `validation_scenarios.py` (8/8). Phase 6 observabilité (22 logs structurés + alertes OVH). Phase 7 doc : 3 nouveaux invariants `I-PII-01`/`I-PROMPT-01`/`I-PROMPT-02` + Pattern `#25` Contradictions inter-blocs. **32 anomalies trouvées au cumul, 16 corrigées**, dont 1 critique (P3-Data-A1 confidence non-float crash) et 1 fuite RGPD subtile (P6-Leak-A1 subject piégé loggé en clair). Bilan complet [`OUTLOOK_BILAN_SESSION_20260508_audit_remediation.md`](../sessions/OUTLOOK_BILAN_SESSION_20260508_audit_remediation.md).
>
...
| **22/05/2026** | [`OUTLOOK_BILAN_SESSION_20260522_images_inline.md`](../sessions/OUTLOOK_BILAN_SESSION_20260522_images_inline.md) | **Session nocode cadrage « Analyse IA images intégrées dans les mails reçus » (1 commit `f0374e1` sur `feat/yvan/frontend`)** : pivot conceptuel affichage → analyse (le plugin Outlook rend les images nativement, le vrai gap est l'analyse pour la génération). Nouveau doc V12 `v12_image intégrée au mail.md` (~350 lignes). Flux 4 étapes : Graph API → Claude Vision → `image_vision_cache` → Bloc I entre Bloc A et Bloc C. 7 décisions Yvan + 4 invariants `I-VISION-01/02/03/04`. Estimation Mika 5-8 j. |
| **23/05/2026** | [`OUTLOOK_BILAN_SESSION_20260523_cadrages_drag_drop_et_sous_dossier.md`](../sessions/OUTLOOK_BILAN_SESSION_20260523_cadrages_drag_drop_et_sous_dossier.md) | **Session double cadrage produit (zéro code, hors WIP permanent)** : (1) Matin — `v12_reponse a partir d'un sous dossier.md` (mail classé). Origine : demande bêta-testeur avocat. Audit technique : backend 100 % compatible IMID-first (refonte N1-N11). 3 ajouts produit + 8 décisions Yvan + 6 invariants `I-CLASSED-01` à `06`. Estimation ~1.5 j Mika. (2) Après-midi — `v12_drag and drop.md`. WIP partiel `V2/dialog.html`+`dialog.js` à compléter. 18 décisions Yvan (Q1-Q15 + C1-C3) + 9 invariants `I-DRAGDROP-01` à `09`. Périmètre V1 élargi : 25 Mo aligné Outlook M365 (upload session Graph) + popup sécurité dédiée + analyse IA des PJ ajoutées (documents → texte, images → Vision Bloc I cohérent images V12). Estimation ~13-16.5 j Mika. **Incohérence détectée** : `v12 - nouveau mail.md` §5.6 contredit C2 23/05 (popup groupée vs immédiate) → action début prochaine session. Pile Mika totale : ~51-62 j. |
| **23/05/2026 (PM)** | [`OUTLOOK_BILAN_SESSION_20260523_PM_transfert_de_mail.md`](../sessions/OUTLOOK_BILAN_SESSION_20260523_PM_transfert_de_mail.md) | **Session cadrage produit transfert de mail V12 (~3 h, 100 % cadrage, zéro code hors WIP)** : nouveau doc `v12_transfert de mail.md` (~700 lignes). Méthodologie « flux étape par étape » demandée par Yvan : 11 étapes déroulées (mail ouvert → clic Transférer → état initial dialog → saisie destinataire → zone PJ permanente → brief optionnel → génération → streaming → affinage → envoi → post-envoi). **40 décisions Yvan tranchées** (5 frictions F1-F3 + 35 questions Q1-Q40). 6 livrables produits (zone PJ permanente remplace popup interruptive `#popupFwdPj`, brief auto adaptatif « ci-dessous » + synthèse adaptative, multi-destinataires + bandeau « Destinataire de référence » réutilisé du chantier nouveau mail, garde forward triple barrière, boutons refine rapides avec undo `[Réponse optimisée]`, threading des réponses via `In-Reply-To`). 8 invariants `I-FORWARD-01` à `08` (nouvelle Catégorie 21 V12_INVARIANTS.md). **Découverte critique** : popup PJ forward affichée mais désarmée côté backend (`_fwdSelectedIndexes` ignoré, Graph inclut toutes les PJ quoi qu'il coche) — gap fonctionnel hérité du portage proto → V2 d'avril 2026, jamais détecté car silencieux UX. Estimation Mika **~8 j**. Pile Mika totale : **~55-66 j sur 7 chantiers V12**. Top commit cadrage `5f1dd27`. |
