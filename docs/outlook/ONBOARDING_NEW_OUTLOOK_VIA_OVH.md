...
> **Dernière mise à jour** : 30/04/2026 PM (incident prod FD leak SQLite résolu : commit `b2d2f73` + `LimitNOFILE=65535` systemd ; voir bilan `OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md`)
...
| **30/04/2026 (PM)** | [`OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md`](../sessions/OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md) | **Session incident prod FD leak (~1h45)** : UptimeRobot alerte 06:11 UTC, prise en main 10:25, fix déployé 10:42. Cause : `Database._conn()` thread-local + ~80 sites `threading.Thread()` daemon transitoires → conn SQLite zombies non fermées (509 db + 508 wal handles à saturation). Fix double : palliatif `LimitNOFILE=65535` systemd + fix root commit `b2d2f73` (dict + thread BG `db-gc` 60s). Validation : 614 → 44 FDs stables après 3 min. Doc cascade : Pattern #21 + I-DB-06 + ROLLBACK Cas 0/4 + PLUS_TARD_VF mise à jour. |
