...
> **Dernière mise à jour** : 30/04/2026 PM étendu (5 commits ce jour : `b2d2f73` FD leak + `8c407b8` doc + `95178cc` fix dialog push undefined + `e419741` fix popup Not Found + `842dc31` endpoint diag db_conns_stats + audit autres leaks ; voir bilan `OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md` Partie 1+2)
...
| **30/04/2026 (PM étendu)** | [`OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md`](../sessions/OUTLOOK_BILAN_SESSION_20260430_PM_incident_fd_leak.md) | **Session incident + 2 fixes UI Yvan + Phase autonomie (~3h, 5 commits)** : (1) UptimeRobot alerte 06:11 UTC FD leak SQLite (`Database._conn()` thread-local + threads daemon transitoires) → fix double `LimitNOFILE=65535` systemd + commit `b2d2f73` (dict + thread BG `db-gc` 60s). (2) Yvan signale Bug B `push undefined` dialog.js:3272 (régression S2 hoisting var) → fix commit `95178cc` (déclaration en TOP) + cache bust v27. (3) Yvan signale Bug A toast "Not Found" popup overlay → fix commit `e419741` (`_backendUrl = window.location.origin` au lieu de localhost hardcodé) + cache bust v15. (4) Phase autonomie : commit `842dc31` endpoint `/api/admin/db_conns_stats` + audit autres leaks (rapport `audit/rapports/2026-04-30_PM_audit_autres_leaks_ressources.md`, 2 leaks pré-beta identifiés HIGH GraphClient + MEDIUM ThreadPoolExecutor). Doc cascade : Pattern #21 + I-DB-06 + ROLLBACK Cas 0/4 + PLUS_TARD_VF + bilan Partie 1+2. |
