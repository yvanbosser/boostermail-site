...
> **Dernière mise à jour** : 05/05/2026 — session **« Échéances V2 SaaS »** (3 commits, top `a34f490`). Audit complet feature → spec consolidée [`SPEC_ECHEANCES_BOOSTERMAIL.md`](../specs_proto/SPEC_ECHEANCES_BOOSTERMAIL.md) (13 sections, source de vérité unique). **Close gaps §10.1+§10.2** : backend `/api/echeances/<id>/relance` + `/mail` portés du proto + frontend `relancer(id)` via mailto: + frontend `voirMail(id)` via modale d'aperçu locale. **Garde-fou nouveau I-SESS-05** dans `audit/INVARIANTS.md` + `cloture_check.sh` pour empêcher sessions futures de retomber dans le piège `OneDrive\Desktop\EasyMail\` (vestige obsolète depuis 12/04). Bilan [`OUTLOOK_BILAN_SESSION_20260505_echeances.md`](../sessions/OUTLOOK_BILAN_SESSION_20260505_echeances.md). **Prochaine session : §10.3 scheduler J-1 ouvré** (vrai chantier ~1 jour, reco endpoint cron `/api/echeances/send_reminders` via cron Linux toutes les heures). Sujet 03/05 antérieur (boucle infinie API) toujours en validation post-fix.
>
