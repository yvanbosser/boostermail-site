...
> - **MEDIUM (à investiguer)** Mise en page du mail reçu (onglet « Mail reçu ») toujours mauvaise sur le mail de Stéphane Dufau. Le résumé, la réponse et la signature sont OK ; seul le rendu du body original est dégradé. **Action** : reproduire avec un mail multi-paragraphes, comparer `data.html_body` (ce que renvoie Graph) à ce qu'affiche `mailBody` après sanitize. Probable que la sanitize regex enlève trop ou que les `\r\n` ne sont pas convertis en `<br>` propre.
