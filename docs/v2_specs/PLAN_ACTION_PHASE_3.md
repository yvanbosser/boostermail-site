...
| 11 | App Registration Azure + Mode Complet (Graph API) | Configurer le client_id pour que le dialog standalone ait les données complètes (body, envoi direct) |
...
| Dialog standalone sans Mode Complet | Outlook Web | Le body du mail reçu n'est pas disponible sans Graph API | Configurer App Registration Azure → l'utilisateur active le Mode Complet |
