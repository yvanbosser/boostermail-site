...
- ✅ Mode Complet activé (admin comptasante.fr a approuvé, Graph API disponible)
...
- ❌ Dialog (écran de réponse) : ne reproduit PAS le proto
  ...
  - Body du mail : texte brut au lieu de HTML (même en Mode Complet pas encore testé)
...
| 4 | Panneau gauche : body HTML (Mode Complet = Graph) | HAUTE | ✅ OK (code existant) | dialog.js |
...
**À finaliser** :
- Se met à jour quand on change de mail (Mode Complet : via bouton #7b ou taskpane ItemChanged)
...
**Mode Complet activé** : Graph API disponible.
...
| N5 | Tester envoi (Mode Complet) | Générer → Relire et envoyer → vérifier que le mail part via Graph | N4 |
...
| W2 | Extension overlay s'affiche | ✅ Déjà fonctionne (Mode Complet détecté) | — |
...
| **Backend + Graph API** | ✅ 95% | ✅ 95% | ✅ 95% | Mode Complet activé, 47+ routes |
...
- **Terminologie** : "Mode Complet" → **Mode Complet** | "Mode Degrade" → **Mode Degrade**
