...
> **Dernière mise à jour** : 25/05/2026 — cadrage **qualité de la réponse par contact V12** livré sur `feat/yvan/frontend` :
> - **Nouveau doc V12** : [`docs/architecture/V12/v12_qualite_reponse_par_contact.md`](architecture/V12/v12_qualite_reponse_par_contact.md) ⭐⭐ — **chantier cadre transverse** centré sur la cible « la réponse générée doit donner l'impression que l'utilisateur l'a écrite lui-même ». Décomposition en **5 niveaux d'analyse de données** qui qualifient chaque génération : (N1) style général de l'utilisateur — 8 critères globaux + 10 dernières corrections globales + recalibrage tous les 50 envois, (N2) style avec le contact + apprentissage par contact — 10 critères structurés + 5 dernières corrections spécifiques + score moyen par contact + leçons apprises capitalisées, (N3) classement Outlook (déjà en place, exposé comme signal pour N5), (N4) classement PJ Windows (cadrage 21/05 en cours, pont vers Bloc K), (N5) **historique intelligent + sujet en cours** — point dur structurant : maintenance d'une table `contact_subjects` (titre, mots-clés, mails rattachés, dossier Outlook, PJ rattachées, statut active/dormant/closed) avec clustering Claude Haiku périodique + identification du sujet du mail courant par cascade 6 tests (référence directe, threading In-Reply-To, dossier Outlook, mots-clés communs, PJ classement, fallback Claude Haiku) + restriction de l'historique injecté au sujet identifié + pondération multi-critère 5 axes des 6 sources. 8 invariants `I-QUALITY-01` à `08` proposés (Catégorie 22). Grille comparative état actuel vs cible pour Michael (statut ✅/🟡/❌/❓ par critère). Annexe Mika 3 vagues (A fondations + B sujets en cours + C injection prompt et tests) + estimation **35-47 j**.
>
