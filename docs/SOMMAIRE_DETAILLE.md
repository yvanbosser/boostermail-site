...
> **Dernière mise à jour** : 25/05/2026 — cadrage **qualité de la réponse — complément V12** livré sur `feat/yvan/frontend` :
> - **Nouveau doc V12** : [`docs/architecture/V12/v12_qualité_réponse_complément.md`](architecture/V12/v12_qualit%C3%A9_r%C3%A9ponse_compl%C3%A9ment.md) ⭐⭐ — **complément à l'existant V12** (pas un cadrage from scratch). Après audit complet de l'existant (V12_CUISINE, V12_SALLE, SPEC_CONTACTS, code `V2/database.py` + `V2/claude_ai.py`), constat : 80 % du périmètre est déjà cadré ou implémenté (table `contact_profiles` à 26 colonnes, `profile_json` enrichi, blocs prompt A/B/C/D/D2/G, tiers confiance 70/50/30, decay 5%/trimestre, schedule re-analyse fixe `[1,2,3,4,5,7,9,13,17,25,50,75,100,150,200]`, anti-inversion greeting `polluted`, multi-tenant `user_id`, purge auto 24 mois, `style_corrections` + `echeances` + `folder_classifications`, classification_history 20e, attachment_folder_history 21e, default_importance R/S/H, `_pj_text_cache` + `image_vision_cache`). Doc recentré sur les **10 écarts réellement manquants** : (E1) score génération +3/+1/0/-2/-3, (E2) climat du fil en cours (nature/sensibilité/émotion/escalade), (E3) détection de dérive du profil contact, (E4) désaveu utilisateur des correction_patterns, (E5) faits factuels extraits du corps des mails (pas que PJ), (E6) sujets en cours par contact (table `contact_subjects` + cascade 5 tests d'identification + fallback Haiku), (E7) cold start formalisé (champ `etat_apprentissage`), (E8) style général utilisateur en DB structurée (au lieu de `style_profile.txt`), (E9) promesses ouvertes séparées des échéances, (E10) profil bidirectionnel (ton/expressions/longueur du contact en miroir). 10 invariants `I-QUALITY-01` à `10` (Catégorie 22 V12_INVARIANTS.md). Annexe Mika 4 phases avec dépendances + estimation **26-35 j** (au lieu de 35-47 j de la spec initiale, après resserrement sur les vrais écarts).
>
> **Précédente** : 23/05/2026 PM — cadrage **transfert de mail V12** livré sur `feat/yvan/frontend` :
> - **Nouveau doc V12** : [`docs/architecture/V12/v12_transfert de mail.md`](architecture/V12/v12_transfert%20de%20mail.md) ⭐⭐ — **chantier dédié transfert de mail** (mode forward). Flux complet en 11 étapes (mail ouvert → clic Transférer → état initial dialog → saisie destinataire → zone PJ permanente → brief optionnel → génération → streaming → affinage → envoi → post-envoi). 6 livrables produits : (L1) zone PJ permanente remplaçant la popup interruptive (PJ originales cochées par défaut + PJ ajoutées via drag-drop ou bouton +, compteur cumulé 25 Mo), (L2) brief auto adaptatif (tonalité-destinataire + synthèse courte/détaillée selon longueur mail + lexique « ci-dessous » corrigé), (L3) multi-destinataires + bandeau « Destinataire de référence » réutilisé du chantier nouveau mail, (L4) garde forward formalisée triple barrière (frontend + 3 routes backend) + tooltip « Veuillez d'abord saisir un destinataire », (L5) boutons refine rapides avec undo `[Réponse optimisée]` (grisé par défaut, devient cliquable après refine = retour version d'origine) + `[Plus court]` + `[Plus travaillé]` + `[Essayer une autre réponse]`, (L6) threading des réponses via `In-Reply-To` (Roland répond → BoosterMail récupère le mail original M. Dupont en contexte). 40 décisions Yvan tranchées (5 frictions + 35 Q1-Q40). 8 invariants `I-FORWARD-01` à `08` (Catégorie 21 V12_INVARIANTS.md). Annexe Mika 9 phases + estimation **8 j**. Résorbe 6 trous structurels V2 actuel dont le plus critique : popup PJ affichée mais désarmée côté backend (`_fwdSelectedIndexes` ignoré, Graph inclut toutes les PJ quoi qu'il coche).
>
> **Précédente** : 22/05/2026 — cadrage **analyse IA des images intégrées dans les mails reçus** livré sur `feat/yvan/frontend` :
> - **Nouveau doc V12** : [`docs/architecture/V12/v12_image intégrée au mail.md`](architecture/V12/v12_image%20int%C3%A9gr%C3%A9e%20au%20mail.md) ⭐⭐ — **chantier dédié analyse IA des images inline**. Pivot conceptuel : le problème n'est pas l'AFFICHAGE (résolu par le plugin Outlook rendu natif) mais l'ANALYSE du contenu visuel pour enrichir la génération de réponse. Flux 4 étapes : Extraction Graph API (filtre `isInline=true` + taille ≥ 5 Ko + position avant signature + cap 3 images) → Analyse Claude Vision (`claude-sonnet-4-20250514`) → Cache `image_vision_cache` (clé `internet_message_id` + hash image, pas de TTL) → Injection **Bloc I** entre Bloc A et Bloc C dans le contexte de génération. Timing : background pour H/VIP (Smart Speculative), parallèle au clic avec timeout 10s pour S/R. 7 décisions Yvan tranchées (D1-D7). 4 invariants créés (`I-VISION-01/02/03/04`). Annexe Mika 4 phases + estimation **5-8 j**.
>
> **Dernière mise à jour précédente** : 21/05/2026 (PM) — cadrage **nouveau mail V12 + fenêtre de rédaction grande/petite** livré sur `feat/yvan/frontend` :
> - **2 nouveaux docs V12** :
>   - [`docs/architecture/V12/v12 - nouveau mail.md`](architecture/V12/v12%20-%20nouveau%20mail.md) ⭐⭐ — **chantier dédié composition de nouveaux mails**. 4 livrables produits : (L1) flux séquentiel cuisine progressive avec préchargement à la sélection autocomplete + 3 préchargements parallèles multi-destinataires + 1er = pilote + bandeau « Destinataire de référence », (L2) popup pioche PJ V12 4 zones (miroir popup classement PJ du 21/05 AM), (L3) refonte R/S/H (supprimé en réception, conservé en compose, nouveau champ `default_importance` profil contact avec pré-sélection auto, S par défaut nouveau contact, pas d'escalade auto mots sensibles, override par clic), (L4) marqueurs `[…]` détection regex + fond gris cliquable + garde-fou Envoyer désactivé strict (pas de bypass). 19 décisions Yvan tranchées. Annexe Mika 8 phases + estimation 14.5 j.
>   - [`docs/architecture/V12/v12 fenetre de rédaction _ grande - petite.md`](architecture/V12/v12%20fenetre%20de%20r%C3%A9daction%20_%20grande%20-%20petite.md) ⭐⭐ — **mode agrandi de la fenêtre de rédaction**. Bouton 🗖 → 90% du dialog, tout disparaît sauf zone rédaction + champ Modifier + flèche retour + boutons refine + 🗕. Disponible en compose ET reply/forward (absorbe résumé Bloc P + mail reçu original). Sortie uniquement par 🗕 (pas Escape, pas double-clic). Pas de mémorisation (toujours mode initial au démarrage). 7 décisions Yvan tranchées. Annexe Mika 5 phases + estimation 4 j.
>
> **Dernière mise à jour précédente** : 21/05/2026 (AM) — cadrage **classement PJ V12** livré sur `feat/yvan/frontend` :
> - **Nouveau doc V12** : [`docs/architecture/V12/v12 _ classement PJ.md`](architecture/V12/v12%20_%20classement%20PJ.md) ⭐⭐ — **chantier dédié classement des pièces jointes**. 3 livrables techniques : (L1) champ `attachment_folder_history` 21e attribut `contact_profiles` (pendant PJ de `classification_history` mail), (L2) popup `smart_paperclip` rallumée à 4 zones (V12 + 4 boulettes historique + recherche + arbo synchronisée), (L3) symétrie complète moteur V12 PJ (Tier 2 + 3a + 3b + top 3 IA → **résorbe PLUS_TARD_VF #32 et #34**). 3 chantiers d'alimentation : onboarding + audit Phase 4 + quotidien incrémental. Moteur V12 INTOUCHÉ. Paths stockés relatifs à la racine PJ Profil. Cycle de vie cohérent fiche contact (purge 24 mois, manually_edited, decay 5%, invalidation cache via wrapper Phase C bis). Annexe checklist Mika 13 sections + estimation 13-18 jours.
>
> **Dernière mise à jour précédente** : 20/05/2026 — cadrage produit majeur livré sur `feat/yvan/frontend` :
...
| **Classement PJ V12 — `attachment_folder_history` + popup `smart_paperclip` rallumée + symétrie moteur** (cadré 21/05 AM) ⭐ | `docs/architecture/V12/v12 _ classement PJ.md` |
| **Nouveau mail V12 — flux séquentiel + popup pioche PJ + refonte R/S/H + marqueurs `[…]`** (cadré 21/05 PM) ⭐ | `docs/architecture/V12/v12 - nouveau mail.md` |
| **Fenêtre de rédaction grande/petite — mode agrandi 90% écran** (cadré 21/05 PM) ⭐ | `docs/architecture/V12/v12 fenetre de rédaction _ grande - petite.md` |
| **Réponse depuis un mail classé (sous-dossier Outlook) — backend déjà compatible + 3 gardes produit** (cadré 23/05) ⭐ | `docs/architecture/V12/v12_reponse a partir d'un sous dossier.md` |
| **Drag and drop PJ V1 — 4 modes compose + 25 Mo upload session + analyse IA documents/images** (cadré 23/05) ⭐ | `docs/architecture/V12/v12_drag and drop.md` |
...
| **`v12 - nouveau mail.md`** ⭐⭐ | **CADRAGE PRODUIT 21/05/2026 PM — Composition de nouveaux mails V12**. 4 livrables : (L1) flux séquentiel cuisine progressive (À → PJ → Brief → Génération) avec préchargement à la sélection autocomplete, 3 préchargements parallèles multi-destinataires, 1er destinataire = pilote, bandeau « Destinataire de référence : `{prenom}` (1ère de la liste). Mail calibré pour ce destinataire. », champs grisés tant que À vide (tooltip *« Saisissez d'abord un destinataire »*). (L2) Popup pioche PJ 4 zones miroir V12 classement PJ — Suggestion IA (top 3 dossiers de `attachment_folder_history`) + 4 boulettes historique chronologique + champ recherche + arbo synchronisée. Multi-destinataires = union pondérée. Cap PJ 5K cache / 2K prompt. Pas de classement post-envoi (mise à jour `attachment_folder_history` uniquement). (L3) Refonte R/S/H : supprimé en réception, conservé en compose uniquement, nouveau champ `default_importance` sur `contact_profiles` (`R|S|H|null`), calcul moyenne pondérée 10 derniers envois, recalcul tous les 5 envois, pré-sélection chip à la sélection du destinataire, S par défaut nouveau contact, pas d'escalade auto mots sensibles, override par clic. (L4) Marqueurs `[…]` : prompt Claude renforcé (instruction crochets carrés pour info manquante), regex frontend `/\[[^\]]+\]/g`, fond gris cliquable + mini-input inline, bouton Envoyer désactivé strict tant que marqueurs présents (pas de bypass). Brief = 1 seule zone (l'éditeur). Annexe Mika 8 phases + estimation 14.5 j. 4 invariants à introduire (I-COMPOSE-01/02/03/04). | `nouveau mail`, `compose`, `cuisine compose`, `popup pioche PJ`, `R/S/H`, `default_importance`, `marqueurs`, `garde-fou Envoyer` |
| **`v12 fenetre de rédaction _ grande - petite.md`** ⭐⭐ | **CADRAGE PRODUIT 21/05/2026 PM — Mode agrandi fenêtre de rédaction**. Pendant UI du chantier nouveau mail. Bouton 🗖 (haut droite zone rédaction) → bascule mode agrandi (90% du dialog). En mode agrandi, tout disparaît SAUF : zone rédaction + champ Modifier + flèche retour ↩ (si refine utilisé) + boutons refine rapide (« Plus court », etc.) + bouton 🗕. Disparaissent : champs destinataire/Cc/Objet, bandeau « Destinataire de référence », trombone, chips R/S/H, bouton Envoyer. En reply/reply_all/forward, absorbe aussi le résumé Bloc P + mail reçu original + boutons mode. Sortie uniquement par 🗕 (pas Escape, pas double-clic). Toujours mode initial au démarrage (pas de mémorisation localStorage). Animation 200ms ease-out. Annexe Mika 5 phases + estimation 4 j. 4 invariants à introduire (I-EDITOR-EXPAND-01/02/03/04). | `fenêtre rédaction`, `mode agrandi`, `90% dialog`, `focus rédactionnel`, `🗖`, `🗕`, `compose+reply+forward` |
| **`v12 _ classement PJ.md`** ⭐⭐ | **CADRAGE PRODUIT 21/05/2026 — Classement des pièces jointes V12**. Chantier dédié pendant PJ du classement mail. 3 livrables : (L1) nouveau champ `attachment_folder_history` 21e attribut `contact_profiles` (synthèse par contact des dossiers PJ fréquents avec `direction_stats {in, out}`, paths relatifs à la racine PJ Profil), (L2) popup `smart_paperclip` rallumée à 4 zones (suggestion V12 + 4 boulettes historique chronologique + champ recherche + arbo synchronisée), (L3) symétrie complète moteur V12 PJ avec activation Tier 2 (matching nom dossier filesystem) + Tier 3a (règle domaine PJ) + Tier 3b (règle cross-contact PJ) + top 3 IA PJ → **résorbe PLUS_TARD_VF #32 (top 3 IA) et #34 (Tiers PJ dormants)**. 3 chantiers d'alimentation : (A) onboarding amorçage in/out, (B) audit complet Phase 4 reconstruction massive avec 1 appel Claude par contact multi-dossier, (C) quotidien hooks H1/H2/H3 incrémental. Cycle de vie cohérent fiche contact (purge 24 mois, manually_edited, decay 5%, invalidation cache via wrapper Phase C bis). Moteur V12 INTOUCHÉ. **Annexe Mika** : checklist 13 sections + ordre d'implémentation 8 phases + estimation 13-18 jours. | `PJ`, `attachment_folder_history`, `smart_paperclip`, `classement pièces jointes`, `tier 2 PJ`, `tier 3 PJ`, `top 3 IA PJ`, `Estelle` |
| **`v12_drag and drop.md`** ⭐⭐ | **CADRAGE PRODUIT 23/05/2026 — Drag and drop de pièces jointes en composition V1**. Surface unique = 4 modes couverts (`new` / `reply` / `reply_all` / `forward`). WIP existant `V2/dialog.html`+`dialog.js` (overlay `#dragDropOverlay` + tableau `_attachedFiles` + handlers dragover/drop) à compléter avec lecture binaire + base64 + payload. **Limites alignées Outlook M365** : 25 Mo cumulés par mail (Q5 23/05) → upload session Graph obligatoire pour fichiers > 3 Mo (chunks 4 Mo). **Blacklist exécutables** (17 extensions) + popup d'alerte sécurité dédiée (pas un toast, Q8 23/05) avec message « pour des raisons de sécurité… veuillez utiliser Outlook directement ». **PJ ajoutée déclenche analyse IA** (Q9 23/05) : popup groupée au clic Générer (avant) ou popup proactive (après génération), régénération avec contexte enrichi. **2 branches d'analyse** (C1 23/05) : documents → extraction texte (`_user_pj_text_cache`), images → Claude Vision → Bloc I (`image_vision_cache_user`, cohérent images V12). **2 placards distincts** pour cache PJ reçues vs ajoutées (C3 23/05). 18 décisions tranchées (Q1-Q15 + C1-C3). 9 invariants à introduire (I-DRAGDROP-01/02/03/04/05/06/07/08/09). **Estimation Mika ~13-16.5 j** (P0 investigation Q4 + P1-P4 drag-drop ≤ 3 Mo + P5 upload session + P6/P6bis pipeline analyse IA documents/images + P7 popup post-génération + P8 tests). Dépendance d'ordre Mika : chantier images V12 doit livrer avant ce chantier pour réutilisation `_describe_image_vision()`. | `drag drop`, `pièces jointes`, `glisser-déposer`, `compose`, `upload session`, `25 Mo`, `Vision`, `popup sécurité`, `_attachedFiles` |
| **`v12_reponse a partir d'un sous dossier.md`** ⭐⭐ | **CADRAGE PRODUIT 23/05/2026 — Génération de réponse depuis un mail classé**. Origine : demande bêta-testeur avocat (workflow par dossier client universel : avocats / comptables / RH / immo). **Audit technique du 22/05** : backend déjà 100% compatible (architecture IMID-first des refontes N1-N11), routes V2 + cache DB + Graph API + frontend dialog + manifest sans filtre Inbox. Contexte B (`search_by_sender` via `/me/messages?$search=from:...`) scanne **tous les dossiers** — propriété structurelle. **Argument commercial** : plus l'utilisateur classe, plus l'historique disponible pour la génération est riche. **3 ajouts produit à coder** : (1) détecter `direction == sent` (calculé via `from_email == user_email`, indépendant du dossier) → désactiver génération avec message option C *« BoosterMail ne génère pas de réponse à vos propres envois. »*, (2) désactiver pour dossiers spéciaux (Brouillons / Corbeille / Indésirables) avec message dédié par dossier — IDs cachés au warmup via `_special_folders_ids`, (3) pré-suggestion classement post-envoi = dossier d'origine du mail (confiance 100%, court-circuit pipeline IA standard, économie 1 appel Claude). Garde technique : `mark_treated` skip si hors Inbox. 8 décisions Yvan tranchées (D1-D8). 6 invariants à introduire (`I-CLASSED-01/02/03/04/05/06`). **Estimation Mika ~1.5 j** (recommandation : intégrer au sprint Nouveau mail V12 pour mutualiser modif `dialog_init`). | `mail classé`, `sous-dossier`, `direction sent`, `dossiers spéciaux`, `pré-suggestion classement`, `avocat`, `Inbox-only`, `parentFolderId` |
