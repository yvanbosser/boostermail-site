...
MODEL_CLASSIFY = "claude-3-5-haiku-20241022"  # Classification = tâche simple, Haiku suffit (10-20x moins cher)
...
Brief ou modification de l'utilisateur = PRIORITAIRE sur tout le reste (style, ton, longueur).
Sans brief explicite, le ghost-writer NE PREND JAMAIS de decision pour l'utilisateur. Il accuse reception, reformule les demandes, et insere des placeholders pour les points qui necessitent un arbitrage. Avec un brief, il suit les directives du brief a la lettre.
...
class ClaudeAssistant:
    ...
    def reload_style(self):
        ...
        global SYSTEM_PROMPT, _style_profile
        if os.path.exists(_style_path):
            with open(_style_path, "r", encoding="utf-8") as f:
                _style_profile = f.read().strip()
            SYSTEM_PROMPT = _build_system_prompt(_style_profile)
            print(f"[claude] Profil de style recharge ({len(_style_profile)} chars)", flush=True)
...
Resume : {cp.get('profile_text', '')}{extras}{confidence_note}
...
⚠️ RÈGLE ABSOLUE : tu DOIS utiliser "{_greeting}" comme ouverture et "{_closing}" comme clôture pour ce correspondant. Ne PAS utiliser d'autres formules (Hello, Hi, Salut, etc.) sauf si le registre est "tutoiement" ET le ton "amical"."""
            ...
            # Pas de profil connu → nouveau correspondant, défaut professionnel
            blocks.append(f"""## D — Nouveau correspondant ({_contact_display}, {to_email})
Aucun profil connu. RÈGLES PAR DÉFAUT STRICTES :
- Registre : **vouvoiement** (OBLIGATOIRE — ne JAMAIS tutoyer un correspondant sans profil)
- Ton : **professionnel**
- Ouverture OBLIGATOIRE : "Bonjour [Prénom]," (ex: "Bonjour Vincent,")
- Clôture OBLIGATOIRE : "Cordialement," ou "Bien cordialement,"
- ⛔ INTERDIT pour ce correspondant : "Salut", "Hello", "Hi", "Cher/Chère", "Coucou", tutoiement (tu/te/ton/ta/tes)
- ⛔ IGNORE les patterns d'ouverture du style profile (section A) — ils concernent d'AUTRES correspondants avec qui l'utilisateur a un registre DIFFÉRENT
- EXCEPTION UNIQUE : si le fil de conversation (bloc A ci-dessous) montre CLAIREMENT que l'utilisateur tutoie CE correspondant dans ses propres messages, tu peux adopter le tutoiement""")
        ...
        if is_first_mail:
            ...
            brief_line = f"\n\n## *** BRIEF DE L'UTILISATEUR (DIRECTIVE PRIORITAIRE — ce sont des INSTRUCTIONS, pas du texte a recopier. Comprends l'intention et redige avec tes propres mots dans le style de l'utilisateur) :\n{clean_brief}" if clean_brief and clean_brief.strip() else ""
...
Retourne uniquement le mail, sans objet ni commentaire."""
        ...
        if is_forward:
            ...
            original_body = incoming_email.get('body', incoming_email.get('body_preview', ''))
...
{incoming_email.get('body', incoming_email.get('body_preview', ''))}{att_line}{project_line}{importance_line}
            ...
            for block in response.content:
                if block.type == "text":
                    ...
                    _DEFAULTS = {
                        ...
                        'power_dynamic': 'pair', 'language': 'fr',
                    }
...
Choisis parmi les DOSSIERS OUTLOOK DISPONIBLES fournis dans le system prompt."""
    ...
    def suggest_pj_folder(self, sender, subject, attachment_names, folder_tree, recent_pj_classifications=None, contact_profile=None, pj_history=None):
        ...
        # Pré-filtrer les dossiers Windows par mots-clés du sujet + historique contact
        _filter_words = set()
...
RÈGLES :
1. Analyse d'abord l'OBJET du mail pour identifier le SUJET RÉEL (quel bien, quelle affaire, quel dossier). Le sujet du mail prime sur l'historique du contact.
...
Choisis parmi les DOSSIERS WINDOWS DISPONIBLES fournis dans le system prompt."""
    ...
    def analyze_correction(self, proposed, sent):
        """Micro-analyse Claude du diff proposed/sent — 1 ligne explicative."""
        try:
            ...
            response = self._create_with_retry(
                ...
                temperature=0.2,
                system="Tu analyses des corrections de style email. Reponds en 1 seule phrase concise.",
                messages=[{"role": "user", "content": f"L'IA proposait :\n\"{prop_short}\"\n\nL'utilisateur a envoye :\n\"{sent_short}\"\n\nResume en 1 phrase ce qui a change et pourquoi (style, ton, longueur, formulation)."}]
            )
            for block in response.content:
                if block.type == "text":
                    return block.text.strip()
