...
class OutlookClient:
    ...
    def save_inline_images(self, entry_id):
        ...
        try:
            ...
            if not html_body or 'cid:' not in html_body:
                return {}
            inline_images = {}
            mail_dir = os.path.join(_TEMP_ATTACHMENTS_DIR, re.sub(r'[\\/:*?"<>|]', '_', entry_id[:40]))
    ...
    def get_sent_emails(self, limit=80):
        """Lit les N derniers mails envoyes (body complet) pour analyse de style."""
        try:
            pythoncom.CoInitialize()
            ns = get_outlook()
            ...
            emails = []
            for i in range(1, min(limit, items.Count) + 1):
                try:
                    m = items.Item(i)
                    ...
                    emails.append({
                        ...
                        'body': body,
                        'date': str(getattr(m, 'SentOn', ''))[:10]
                    })
                except Exception:
                    pass
            print(f"[outlook] Sent: {len(emails)} emails lus", flush=True)
            return emails
        except Exception as e:
    ...
    def _inject_body(self, mail_item, body_text):
        """Injecte le body HTML au début du mail (avant le contenu original/quoté)."""
        tag = '<br><br><span style="color:#999;font-size:11px;">— Généré avec EasyMail</span>'
    ...
    def send_reply(self, entry_id, body_text, cc=None, attachment_paths=None):
        ...
        try:
            ...
            if cc:
                reply.CC = cc
            self._inject_body(reply, body_text)
            if attachment_paths:
    ...
    def send_reply_all(self, entry_id, body_text, cc=None, attachment_paths=None):
    ...
    def send_forward(self, entry_id, body_text, to_email, cc=None, attachment_paths=None):
        ...
        try:
            ...
            if cc:
                fwd.CC = cc
            self._inject_body(fwd, body_text)
            if attachment_paths:
    ...
    def send_new_email(self, to_email, subject, body_text, cc=None, attachment_paths=None):
